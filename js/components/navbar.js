    ...
    <div class="app-shell ${opts.bottom ? "with-bottom" : ""}">
      <header class="topbar" id="mainTopbar">
        ...
        <nav class="desktop-nav">
          ...
          <button data-go="catalogue">${i18n.navMarket()}</button>
          <button data-go="requests">${i18n.navRequests()}</button>
          <button data-go="prices">${i18n.navPrices()}</button>
          <button data-go="dashboard">${i18n.navDashboard()}</button>
        </nav>
      ...
      ${content}
      ${opts.bottom ? bottomNav(feedBadge) : ""}
      <div id="toastStack" class="toast-stack"></div>
    </div>`;
}

export function bottomNav(feedBadge = 0) {
  ...
  return `<nav class="bottom-nav" id="bottomNav">
    ...
    <button class="${state.page === 'requests' ? 'active' : ''}" data-go="requests">${icon("heart")}<span>${i18n.navRequestsShort()}</span></button>
